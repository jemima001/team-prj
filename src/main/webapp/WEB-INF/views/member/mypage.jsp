...
<script>
	$(document).ready(
			function() {
				...
				$("#modifySubmitButton2").click(
						function(e) {
							e.preventDefault();
							const form2 = $("#form2");

							// input 값 옮기기
							form2.find("[name=password]").val(
									$("#passwordInput1").val());
							form2.find("[name=email]").val(
									$("#emailInput1").val());
							form2.find("[name=nickName]").val(
									$("#nickNameInput1").val());
...
<body>
	...
	<my:mypagenavbar current="mypage"></my:mypagenavbar>
				...
				<div class="col-12 col-lg-6">
					...
					<div>
						<label for="nameInput1" class="form-label"> 이름 </label> <input
							...
							value="${member.name }" readonly /> <label for="idInput1"
							class="form-label"> 아이디 </label> <input id="idInput1"
							class="form-control" type="text" value="${member.id }" readonly />

						<label for="passwordInput1" class="form-label"> 암호 </label> <input
							class="form-control" id="passwordInput1" type="text" value="" />

						<label for="passwordInput2" class="form-label"> 암호확인 </label> <input
							class="form-control" id="passwordInput2" type="text" value="" />
						...
						<div class="input-group">
							<input class="form-control" id="nickNameInput1" type="text"
								value="${member.nickName }" />
							<button class="btn btn-secondary" id="nickNameCheckButton1"
					...
					<div class="mt-3">
						<button class="btn btn-secondary" id="modifySubmitButton1"
							data-bs-toggle="modal" data-bs-target="#modal2" disabled>수정</button>
						<button class="btn btn-danger" data-bs-toggle="modal"
							data-bs-target="#modal1">삭제</button>
					</div>
				</div>
			</div>
		</div>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="form1" action="${appRoot }/member/remove" method="post">
						<input type="hidden" value="${member.id }" name="id" /> <label
							for="passwordInput3" class="form-label"> 암호 </label> <input
							class="form-control" id="passwordInput3" type="text"
							name="password" />
					</form>
				</div>
				...
				<div class="modal-body">
					<form id="form2" action="${appRoot }/member/modify" method="post">
						<input type="hidden" value="${member.id }" name="id" /> <input
							type="hidden" name="password" /> <input type="hidden"
							name="email" /> <input type="hidden" name="nickName" /> <label
							for="passwordInput4" class="form-label"> 기존 암호 </label> <input
							class="form-control" id="passwordInput4" type="text"
							name="oldPassword" />
					</form>
				</div>
				<div class="modal-footer">
